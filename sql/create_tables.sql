create table if not exists click_log(page_name TEXT, section text, rank INT, query text, ip text,
    ...
    referrer text, click_time int, dist text, useragent text);
...
query_time int, dist text, useragent text);

create table if not exists page_visit_log(page_id int, ip text, platform text, browser text, version text, language text, visit_time int, dist text, referrer text, useragent text);
